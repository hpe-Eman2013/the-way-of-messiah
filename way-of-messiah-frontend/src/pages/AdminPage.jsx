...
export default function AdminPage() {
  ...
  const handleBulkAction = async (action) => {
    ...
    try {
      ...
      setTestimonies((prev) => {
        if (action === "delete")
          return prev.filter((t) => !selectedIds.includes(t._id));
        if (action === "approve")
          return prev.map((t) =>
            selectedIds.includes(t._id) ? { ...t, approved: true } : t
          );
        if (action === "disapprove")
          return prev.map((t) =>
            selectedIds.includes(t._id) ? { ...t, approved: false } : t
          );
        return prev;
      });
      setSelectedIds([]);
    ...
    <div className="min-h-screen bg-gray-100 text-black">
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="mb-4 flex items-center gap-4">
          <input
            type="checkbox"
            checked={selectedIds.length === testimonies.length}
